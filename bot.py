...
PREMIUM_TEXT_HINTS: dict[str, str] = {
    "⚙️": "settings",
    "⚙": "settings",
    "👤": "profile",
    "👥": "people",
...
BUTTON_ICON_HINTS: dict[str, str | None] = {
    "⚙️": "settings",
    "⚙": "settings",
    "🎛": "settings",
    "🔥": "settings",
...
def premium_html(text: str) -> str:
    if not text:
        return text
    result = text
    for token in _PREMIUM_TEXT_TOKENS:
        result = result.replace(token, premium_emoji(PREMIUM_TEXT_HINTS[token], token))
    return result


def _sanitize_button_text(text: str) -> str:
...
def _normalize_button(text: str) -> tuple[str, str | None]:
    ...
    for token in _BUTTON_TOKENS:
        if raw.startswith(token):
            premium_key = BUTTON_ICON_HINTS[token]
            if premium_key:
                icon_custom_emoji_id = PE.get(premium_key)
            break
    cleaned = _sanitize_button_text(raw)
    if raw.startswith(("◀️", "◁")):
        cleaned = f"◁ {cleaned}".strip()
    return cleaned or raw, icon_custom_emoji_id


def InlineKeyboardButton(*, text: str, icon_custom_emoji_id: str | None = None, **kwargs):
    normalized_text, auto_icon = _normalize_button(text)
    return AiogramInlineKeyboardButton(
        text=normalized_text,
        icon_custom_emoji_id=icon_custom_emoji_id or auto_icon,
        **kwargs,
    )


def KeyboardButton(*, text: str, icon_custom_emoji_id: str | None = None, **kwargs):
    normalized_text, auto_icon = _normalize_button(text)
    return AiogramKeyboardButton(
        text=normalized_text,
        icon_custom_emoji_id=icon_custom_emoji_id or auto_icon,
        **kwargs,
    )
...
# Состояние группового AI-режима: {chat_id: True/False}
group_ai_modes: dict[int, bool] = {}

# Память разговора для групп: {chat_id: deque([{role, content}, ...])}
group_ai_memory: dict[int, deque] = {}

# Rate-limit для групп: {chat_id: timestamp последнего ответа}
_group_ai_last_reply: dict[int, float] = {}

# Интервал между ответами в группе (секунды)
GROUP_AI_RATE_LIMIT = 2.5

# Системный промпт для группового режима
GROUP_AI_SYSTEM_PROMPT = (
    f"Ты — умный и прямолинейный AI-ассистент в групповом чате. "
    "Отвечай чётко, без лишней воды и без слащавых вступлений. "
    "Говори по делу. Будь полезным. "
    "Отвечай на языке пользователя (по умолчанию — русский). "
    "Помни контекст предыдущих сообщений в этом чате."
)

# Максимальная длина истории группового чата (сообщений)
GROUP_AI_MEMORY_SIZE = 30


def is_group_chat(message: "Message") -> bool:
    """Проверяет, что сообщение из группы/супергруппы."""
    return message.chat.type in ("group", "supergroup")


def is_group_ai_enabled(chat_id: int) -> bool:
    """Включён ли AI-режим в данной группе."""
    return group_ai_modes.get(chat_id, False)


async def is_group_admin(chat_id: int, user_id: int) -> bool:
    """Проверяет является ли пользователь админом группы (через API или ADMIN_IDS)."""
    ...
    try:
        ...
        logging.warning(f"[GROUP AI] is_group_admin check failed: {e}")
        return False


def get_group_memory(chat_id: int) -> deque:
    """Возвращает (или создаёт) историю сообщений для группы."""
    if chat_id not in group_ai_memory:
        group_ai_memory[chat_id] = deque(maxlen=GROUP_AI_MEMORY_SIZE)
    return group_ai_memory[chat_id]


def group_ai_rate_ok(chat_id: int) -> bool:
    """Проверяет rate-limit для группы. True = можно отвечать."""
    import time as _t
    now = _t.time()
    last = _group_ai_last_reply.get(chat_id, 0)
    if now - last >= GROUP_AI_RATE_LIMIT:
        _group_ai_last_reply[chat_id] = now
        return True
    return False


def should_group_ai_respond(message: "Message", bot_username: str) -> bool:
    """
    Определяет нужно ли боту отвечать на сообщение в группе.
    Триггеры:
      A) /ai текст
      B) @botname упомянут
      C) реплай на сообщение бота
    """
    text = (message.text or "").strip()
    # A) команда /ai
    if text.lower().startswith("/ai ") or text.lower() == "/ai":
        return True
    # B) упоминание @botname
...
def extract_group_ai_text(message: "Message", bot_username: str) -> str:
    """Извлекает чистый текст из группового сообщения (убирает /ai и @botname)."""
    text = (message.text or "").strip()
    # Убираем /ai в начале
    ...
    if bot_username:
        import re as _re
        text = _re.sub(rf"@{_re.escape(bot_username)}", "", text, flags=_re.IGNORECASE).strip()
    return text or "Привет!"
...
async def _accept_terms_cb(callback: CallbackQuery):
    ...
    await callback.message.answer(premium_emoji("smile", "🙂"), parse_mode="HTML", reply_markup=main_reply_kb(uid))
...
def _welcome_text(name: str, tok: int = 0, is_admin: bool = False, ref_bonus_msg: str = "") -> str:
    admin_badge = "  ⚙️ <b>ADMIN</b>" if is_admin else ""
...
async def cmd_menu(message: Message):
    ...
    if not await require_subscription(message):
        return
    await message.answer(premium_html("🏠 <b>Главное меню:</b>"), parse_mode="HTML", reply_markup=main_reply_kb(uid))
...
async def cmd_ai(message: Message):
    await message.answer(
        premium_html("💬 <b>Режим диалога с ИИ</b>\n\nПросто напиши свой вопрос!"),
...
@dp.message(F.text.in_({"📋 История", "История"}))
...
async def _show_profile(msg_or_cb, uid: int):
    ...
    doc_btn = f"📄 Документы · {'вкл' if feats.get('doc_analysis') else 'выкл'}"
    auto_btn = f"🤖 Авто · {'вкл' if auto_model else 'выкл'}"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        mode_row1,
        mode_row2,
...
@dp.message(F.text.in_({"🧹 Очистить память", "Очистить память"}))
...
@dp.message(F.text.in_({"💬 Написать", "Написать"}))
...
@dp.message(F.text.in_({"🎨 Создать", "Создать"}))
async def rb_sozdat(message: Message):
    ...
    await message.answer(premium_html("🎨 <b>Создать</b>\n\nВыбери что хочешь сделать:"),
                         parse_mode="HTML", reply_markup=kb)


@dp.message(F.text.in_({"🏠 Главная", "Главная"}))
...
@dp.message(F.text.in_({"💎 Подписка", "Подписка"}))
...
# ==================================================================
# 🤖 ГРУППОВОЙ AI-РЕЖИМ — КОМАНДЫ
# ==================================================================

@dp.message(Command("enable_ai"))
async def cmd_enable_ai(message: Message):
    """Включить AI-режим в группе. Только для админов."""
    if not is_group_chat(message):
        await message.answer("⚠️ Эта команда работает только в группах.")
        return
    uid     = message.from_user.id
    chat_id = message.chat.id
    if not await is_group_admin(chat_id, uid):
        await message.answer("🚫 Только администраторы группы могут включать AI-режим.")
        return
    group_ai_modes[chat_id] = True
    # Сбрасываем память при включении (опционально)
    group_ai_memory[chat_id] = deque(maxlen=GROUP_AI_MEMORY_SIZE)
    await message.answer(
        "✅ <b>AI-режим включён!</b>\n\n"
        "🤖 Теперь я отвечаю на сообщения в этом чате.\n\n"
        "<b>Как обратиться ко мне:</b>\n"
        "• <code>/ai ваш вопрос</code>\n"
        f"• Упомянуть <code>@{(await bot.get_me()).username}</code>\n"
        "• Ответить на моё сообщение\n\n"
        "Выключить: /disable_ai (только админ)\n"
        "Статус: /status",
        parse_mode="HTML"
    )
...
async def cmd_disable_ai(message: Message):
    """Выключить AI-режим в группе. Только для админов."""
    if not is_group_chat(message):
        await message.answer("⚠️ Эта команда работает только в группах.")
        return
    uid     = message.from_user.id
    chat_id = message.chat.id
    if not await is_group_admin(chat_id, uid):
        await message.answer("🚫 Только администраторы группы могут выключать AI-режим.")
        return
    group_ai_modes[chat_id] = False
    await message.answer(
        "🔴 <b>AI-режим выключен.</b>\n\n"
        "Я больше не отвечаю на сообщения в этом чате.\n"
        "Включить снова: /enable_ai (только админ)",
        parse_mode="HTML"
    )
...
async def cmd_status(message: Message):
    """Показать статус AI-режима."""
    if not is_group_chat(message):
        await message.answer("ℹ️ Команда /status актуальна в групповых чатах.")
        return
    chat_id = message.chat.id
    enabled = is_group_ai_enabled(chat_id)
    mem_len = len(get_group_memory(chat_id))
    status_icon = "🟢" if enabled else "🔴"
    status_text = "ВКЛЮЧЁН" if enabled else "ВЫКЛЮЧЁН"
    bot_info = await bot.get_me()
    await message.answer(
        f"{status_icon} <b>AI-режим: {status_text}</b>\n\n"
        f"💬 Сообщений в памяти: <code>{mem_len}</code>\n\n"
        f"<b>Как обратиться к боту:</b>\n"
        f"• <code>/ai вопрос</code>\n"
        f"• <code>@{bot_info.username} вопрос</code>\n"
        f"• Ответить на сообщение бота\n\n"
        f"<i>Управление (только админы): /enable_ai · /disable_ai</i>",
        parse_mode="HTML"
    )
...
async def cmd_help_group(message: Message):
    ...
    if not is_group_chat(message):
        # В личном чате — молчим, не мешаем основному функционалу
        return
    bot_info = await bot.get_me()
    await message.answer(
        "📖 <b>Как пользоваться AI-ботом в группе</b>\n\n"
        "<b>Для всех участников:</b>\n"
        "• <code>/ai вопрос</code> — задать вопрос\n"
        f"• <code>@{bot_info.username} вопрос</code> — упомянуть бота\n"
        "• Ответить на сообщение бота — продолжить диалог\n\n"
        "<b>Только для администраторов:</b>\n"
        "• /enable_ai — включить AI-режим\n"
        "• /disable_ai — выключить AI-режим\n\n"
        "<b>Для всех:</b>\n"
        "• /status — статус AI-режима\n"
        "• /clear_group — очистить память группы (только админ)\n\n"
        "ℹ️ Бот помнит историю разговора в группе.",
        parse_mode="HTML"
    )


@dp.message(Command("clear_group"))
async def cmd_clear_group(message: Message):
    """Очистить память группового чата. Только для админов."""
    if not is_group_chat(message):
        return
    uid     = message.from_user.id
    chat_id = message.chat.id
    if not await is_group_admin(chat_id, uid):
        await message.answer("🚫 Только администраторы могут очищать память.")
        return
    group_ai_memory[chat_id] = deque(maxlen=GROUP_AI_MEMORY_SIZE)
    await message.answer("🧹 <b>Память группового чата очищена!</b>", parse_mode="HTML")


# ==================================================================
# 🤖 ГРУППОВОЙ AI-РЕЖИМ — ОБРАБОТКА СООБЩЕНИЙ
# ==================================================================
...
async def handle_group_message(message: Message):
    ...
    Отвечает если:
      - AI-режим включён в этой группе
      - Сообщение является триггером (A/B/C)
      - Соблюдён rate-limit
    """
    chat_id = message.chat.id
    uid     = message.from_user.id

    # Игнорируем если AI-режим выключен
    ...
    # Проверяем триггер
    if not should_group_ai_respond(message, bot_uname):
        return

    # Rate-limit
    if not group_ai_rate_ok(chat_id):
        return

    # Извлекаем текст
    user_text = extract_group_ai_text(message, bot_uname)
    if not user_text:
        return

    # Имя пользователя для контекста
    user_name = message.from_user.first_name or f"Пользователь {uid}"

    # Берём память группы
    mem = get_group_memory(chat_id)

    # Добавляем сообщение пользователя с именем для контекста
    mem.append({"role": "user", "content": f"[{user_name}]: {user_text}"})

    # Строим список сообщений
    query_msgs = [{"role": "system", "content": GROUP_AI_SYSTEM_PROMPT}] + list(mem)

    # Выбираем модель — claude_haiku как дефолт для групп (быстрый)
    group_model = "claude_haiku"
    if group_model in disabled_models:
        group_model = next(
            (k for k in ["claude_haiku", "gemini_20_flash", "gemini_25_flash", "deepseek_v3_sq"]
             if k in MODELS and k not in disabled_models),
            "claude_haiku"
        )
    ...
    try:
        async with ChatActionSender.typing(bot=bot, chat_id=chat_id):
            ans = await call_chat(query_msgs, group_model, max_tokens=1000)
    except Exception as e:
        logging.error(f"[GROUP AI] call_chat error: {e}")
        try:
            await message.reply(f"❌ Ошибка AI: {str(e)[:150]}")
        except Exception:
            pass
        return
    ...
    # Отправляем ответ (реплаем на сообщение пользователя)
    try:
        html_ans = md_to_html(ans)
        chunks = [html_ans[i:i+4000] for i in range(0, len(html_ans), 4000)]
        for ci, chunk in enumerate(chunks):
            if ci == 0:
        ...
        try:
            await message.reply(ans[:4000])
        except Exception as e2:
            logging.error(f"[GROUP AI] send error: {e2}")
