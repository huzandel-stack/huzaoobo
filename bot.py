...
async def handle_text(message: Message, state: FSMContext):
    ...
    if uid in last_photo and last_photo[uid].get("photo_context") and not last_photo[uid].get("fusion_pending"):
        ...
        # ── РАСШИРЕННАЯ ЭВРИСТИКА follow-up ──────────────────────────────────
        # Короткий вопрос с уточнением — скорее всего тоже к фото
        is_short_followup = (len(txt_lower_check) < 80 and any(
            w in txt_lower_check for w in ["а если", "а что если", "почему", "как это", "зачем",
                                            "можно ли", "покажи", "объясни"]
        ))

        # ── НОВОЕ: любой вопрос пока photo_context «свежий» (не старше 5 мин) ──
        # Пользователь только что получил ответ на фото и продолжает диалог.
        # Удаляем жёсткий «else: pop» — контекст живёт, пока не придёт
        # явно несвязанная тема или пользователь не очистит память.
        _photo_ts = last_photo[uid].get("ts", 0)
        import time as _time_fix
        _photo_is_fresh = (_time_fix.time() - _photo_ts) < 300  # 5 минут

        # Если вопрос похож на follow-up ИЛИ фото «свежее» < 5 мин — отвечаем в контексте фото
        if is_followup_cmd or is_short_followup or _photo_is_fresh:
            ...
            # Явно несвязанная тема И фото уже «старое» — сбрасываем контекст фото
            last_photo.pop(uid, None)
    ...
    async with ChatActionSender.typing(bot=bot, chat_id=message.chat.id):
        ...
        try:
            ...
            last_photo[uid] = {
                ...
                "ts": _time_global.time(),   # Временная метка для follow-up окна
            }
            if uid not in user_memory:
                user_memory[uid] = deque(maxlen=20)
