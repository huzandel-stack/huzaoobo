...
# ══════════════════════════════════════════════════════════════
# 🎮 ГЕЙМИФИКАЦИЯ: миссии, достижения, XP, стрики
# ══════════════════════════════════════════════════════════════
import json as _game_j
...
# Достижения
ACHIEVEMENTS = [
    {"id": "first_request",  "icon": "🎯", "name": "Первый шаг",        "desc": "Сделай первый запрос",     "req": 1,   "type": "requests"},
    {"id": "ten_requests",   "icon": "🔥", "name": "Разогрев",           "desc": "10 запросов",              "req": 10,  "type": "requests"},
    {"id": "fifty_requests", "icon": "💪", "name": "Постоянный клиент",  "desc": "50 запросов",              "req": 50,  "type": "requests"},
    {"id": "hundred",        "icon": "💎", "name": "Сотка",              "desc": "100 запросов",             "req": 100, "type": "requests"},
    {"id": "ai_addict",      "icon": "😈", "name": "AI зависимый",       "desc": "200 запросов",             "req": 200, "type": "requests"},
    {"id": "first_image",    "icon": "🎨", "name": "Художник",           "desc": "Первая генерация картинки","req": 1,   "type": "images"},
    {"id": "ten_images",     "icon": "🖼",  "name": "Галерея",            "desc": "10 генераций картинок",    "req": 10,  "type": "images"},
    {"id": "first_music",    "icon": "🎵", "name": "Музыкант",           "desc": "Первая генерация музыки",  "req": 1,   "type": "music"},
    {"id": "streak_3",       "icon": "🌟", "name": "3 дня подряд",       "desc": "Streak 3 дня",             "req": 3,   "type": "streak"},
    {"id": "streak_7",       "icon": "🏆", "name": "Неделя мастерства",  "desc": "Streak 7 дней",            "req": 7,   "type": "streak"},
]

# uid → {xp, streak, last_active_date, achievements: [...], daily_missions_done: [...]}
user_gamification: dict = {}

def _get_game(uid: int) -> dict:
    if uid not in user_gamification:
        user_gamification[uid] = {
            "xp": 0, "streak": 0, "last_active_date": "",
            "achievements": [], "daily_missions_done": [],
            "today_chat": 0, "today_image": 0, "today_music": 0,
            "today_date": "",
        }
    return user_gamification[uid]

def _update_streak(g: dict):
    import datetime
    today = datetime.date.today().isoformat()
    yesterday = (datetime.date.today() - datetime.timedelta(days=1)).isoformat()
    if g["last_active_date"] == today:
        return
    if g["last_active_date"] == yesterday:
        g["streak"] = g.get("streak", 0) + 1
    elif g["last_active_date"] != today:
        g["streak"] = 1
    g["last_active_date"] = today

def _check_achievements(uid: int, prof: dict, g: dict):
    import datetime
    total_req = prof.get("requests", 0)
    total_img = prof.get("generations", 0)
    total_mus = prof.get("total_music", 0) if "total_music" in prof else user_limits.get(uid, {}).get("music_used", 0)
    streak    = g.get("streak", 0)
    earned    = set(g.get("achievements", []))
    for ach in ACHIEVEMENTS:
        if ach["id"] in earned:
            continue
        t = ach["type"]
        v = 0
        if t == "requests": v = total_req
        elif t == "images":  v = total_img
        elif t == "music":   v = total_mus
        elif t == "streak":  v = streak
        if v >= ach["req"]:
            earned.add(ach["id"])
            g["xp"] = g.get("xp", 0) + 20
    g["achievements"] = list(earned)

def _get_user_type(uid: int, prof: dict) -> str:
    """Определяет тип пользователя на основе поведения."""
    feats = user_features.get(uid, {})
    mode = feats.get("answer_mode", "fast")
    if mode == "dev":
        return "coder"
    total = prof.get("requests", 0)
    imgs = prof.get("generations", 0)
    if imgs > total * 0.3:
        return "creative"
    if total > 20:
        return "power"
...
USER_TYPE_LABELS = {
    "coder":    {"icon": "💻", "label": "Кодер",        "color": "#0a84ff"},
    "creative": {"icon": "🎨", "label": "Креатив",      "color": "#ff6b35"},
    "power":    {"icon": "⚡",  "label": "Power User",   "color": "#ffd60a"},
    "learner":  {"icon": "🎓", "label": "Исследователь","color": "#30d158"},
    "business": {"icon": "💼", "label": "Бизнес",       "color": "#bf5af2"},
}

def _daily_missions(uid: int, g: dict, prof: dict) -> list:
    """Возвращает миссии дня — 3 штуки с прогрессом."""
    import datetime
    today = datetime.date.today().isoformat()
    total = prof.get("requests", 0)
    done  = g.get("daily_missions_done", [])
    today_chat  = g.get("today_chat",  0) if g.get("today_date") == today else 0
    today_image = g.get("today_image", 0) if g.get("today_date") == today else 0
    # Пересчёт сегодня
    if g.get("today_date") != today:
        today_chat = today_image = 0

    missions = [
        {
            "id": "daily_chat", "icon": "💬",
            "name": "5 запросов за день",
            "current": min(today_chat, 5), "target": 5,
            "xp": 15, "done": today_chat >= 5,
        },
        {
            "id": "daily_image", "icon": "🎨",
            "name": "Сгенерируй картинку",
            "current": min(today_image, 1), "target": 1,
            "xp": 10, "done": today_image >= 1,
        },
        {
            "id": "daily_streak", "icon": "🔥",
            "name": "Сохраняй streak",
            "current": min(g.get("streak", 0), 1), "target": 1,
            "xp": 5, "done": g.get("streak", 0) >= 1,
        },
    ]
    return missions

def _xp_to_level(xp: int) -> tuple:
    """Возвращает (level, xp_in_level, xp_for_next, level_name)."""
    thresholds = [
        (0,    "🌱 Новичок"),
        (50,   "⭐ Начинающий"),
        (150,  "🔥 Активный"),
    ...
    level = 0
    name  = thresholds[0][1]
    for i, (thr, lname) in enumerate(thresholds):
        if xp >= thr:
            level = i
            name  = lname
    nxt_thr = thresholds[level + 1][0] if level + 1 < len(thresholds) else thresholds[-1][0]
    cur_thr = thresholds[level][0]
    xp_in   = xp - cur_thr
    xp_need = nxt_thr - cur_thr
    return level, xp_in, xp_need, name

def _activity_heatmap(uid: int) -> list:
    """Последние 7 дней активности (0-5)."""
    import datetime
    # Здесь используем имеющиеся данные — в реальности нужна БД
    g = _get_game(uid)
    prof = user_profiles.get(uid, {})
    total = prof.get("requests", 0)
    streak = g.get("streak", 0)
    # Симулируем на основе streak и общей активности
    base = min(total // 7, 5)
    days = []
    for i in range(7):
        offset = 6 - i
        if offset < streak:
            activity = max(1, min(5, base + _random.randint(-1, 1)))
        else:
            activity = max(0, min(3, base - 1 + _random.randint(-1, 1))) if total > 0 else 0
        date = (datetime.date.today() - datetime.timedelta(days=offset))
        days.append({"date": date.isoformat(), "day": ["Пн","Вт","Ср","Чт","Пт","Сб","Вс"][date.weekday()], "value": activity})
    return days

async def api_dashboard_handler(request: aiohttp_web.Request) -> aiohttp_web.Response:
    """GET /api/dashboard?uid=... — полные данные для AI Dashboard."""
    import json as _j, urllib.parse as _up, datetime
    headers = {
        "Access-Control-Allow-Origin": "*",
        "Content-Type": "application/json",
    }
    ...
    if not uid:
        try:
            ...
            pass
    if not uid:
        return aiohttp_web.Response(text=_j.dumps({"ok": False, "error": "No uid"}), headers=headers)
    ...
    # Обновляем streak
    _update_streak(g)
    # Добавляем XP за активность
    total_req = prof.get("requests", 0)
    if g.get("xp", 0) == 0 and total_req > 0:
        g["xp"] = total_req * 2  # retroactive XP
    _check_achievements(uid, prof, g)

    sub_active = has_active_sub(uid)
    lv, xp_in, xp_need, lv_name = _xp_to_level(g.get("xp", 0))
    user_type   = _get_user_type(uid, prof)
    ut_info     = USER_TYPE_LABELS.get(user_type, USER_TYPE_LABELS["learner"])
    missions    = _daily_missions(uid, g, prof)
    achievements_full = []
    earned_ids  = set(g.get("achievements", []))
    for ach in ACHIEVEMENTS:
        achievements_full.append({**ach, "earned": ach["id"] in earned_ids})
    ...
    streak = g.get("streak", 0)
    streak_status = ""
    if streak >= 7:   streak_status = f"🏆 {streak} дней подряд — легенда!"
    elif streak >= 3: streak_status = f"🔥 Активен {streak} дня подряд"
    elif streak >= 1: streak_status = f"⚡ Активен {streak} день подряд"
    else:             streak_status = "😴 Давно не заходил"

    # Любимый тип запросов
    imgs = prof.get("generations", 0)
    fav_type = "💬 Чат"
    if imgs > total_req * 0.4:  fav_type = "🎨 Генерации"
    elif total_req > 10:
        feats = user_features.get(uid, {})
        m = feats.get("answer_mode", "fast")
        if m == "dev": fav_type = "💻 Код"
        elif m == "deep": fav_type = "🧠 Анализ"

    # Пик активности (заглушка — без БД нет реальных данных по часам)
    peak_hour = "20:00–22:00"

    # Умная подсказка
    smart_tip = ""
    if user_type == "coder":  smart_tip = "Ты часто пишешь код → держи DeepSeek R1 под рукой"
    elif user_type == "creative": smart_tip = "Ты любишь генерации → попробуй FLUX 3D"
    elif streak >= 3:         smart_tip = f"🔥 Streak {streak} дней! Не останавливайся"
    else:                     smart_tip = "Задай вопрос ИИ — получи XP и открой достижения"
    ...
    return aiohttp_web.Response(
        text=_j.dumps({
            ...
            # Профиль
            "uid":          uid,
            "name":         prof.get("name", "Пользователь"),
            "join_date":    prof.get("joined", ""),
            "user_type":    user_type,
            "user_type_icon":  ut_info["icon"],
            "user_type_label": ut_info["label"],
            "user_type_color": ut_info["color"],
            # XP и уровень
            "xp":           g.get("xp", 0),
            "level":        lv,
            "level_name":   lv_name,
            "xp_in_level":  xp_in,
            "xp_for_next":  xp_need,
            "xp_percent":   round(xp_in / max(xp_need, 1) * 100),
            # Streak
            "streak":       streak,
            ...
            # Подписка
            "has_sub":      sub_active,
            "sub_plan":     sub_plan_label(uid) if sub_active else "",
            "sub_expires":  sub_expires_str(uid) if sub_active else "",
            # Статистика
            ...
            # Цель дня
            "daily_goal":      5,
            "daily_progress":  min(today_chat, 5),
            # Геймификация
            "missions":        missions,
            ...
            "smart_tip":       smart_tip,
            # Активность (7 дней)
            "activity_week":   _activity_heatmap(uid),
            # Модель и режим
            "current_model":   MODELS.get(user_models.get(uid, DEFAULT_MODEL), {}).get("label", ""),
            "answer_mode":     user_features.get(uid, {}).get("answer_mode", "fast"),
        }, ensure_ascii=False),
        headers=headers,
    )


async def api_dashboard_event_handler(request: aiohttp_web.Request) -> aiohttp_web.Response:
    """POST /api/dashboard/event — обновить XP/streak после действия."""
    import json as _j, urllib.parse as _up, datetime
    headers = {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "POST, OPTIONS",
    ...
    try:
        ...
        return aiohttp_web.Response(text=_j.dumps({"ok": False, "error": "bad json"}), status=400, headers=headers)

    uid        = int(data.get("uid", 0))
    event_type = data.get("type", "chat")  # chat | image | music | code
    if not uid:
        return aiohttp_web.Response(text=_j.dumps({"ok": False, "error": "no uid"}), headers=headers)
    ...
    today = datetime.date.today().isoformat()
    _update_streak(g)
    xp_gain = XP_REWARDS.get(event_type, 2)
    g["xp"] = g.get("xp", 0) + xp_gain
    # Обновляем сегодняшние счётчики
    if g.get("today_date") != today:
        g["today_chat"]  = 0
        g["today_image"] = 0
        g["today_date"]  = today
    if event_type == "chat":   g["today_chat"]  = g.get("today_chat",  0) + 1
    if event_type == "image":  g["today_image"] = g.get("today_image", 0) + 1

    prof = user_profiles.get(uid, {})
    _check_achievements(uid, prof, g)

    lv, xp_in, xp_need, lv_name = _xp_to_level(g["xp"])
    return aiohttp_web.Response(
        text=_j.dumps({
            "ok": True, "xp": g["xp"], "xp_gain": xp_gain,
            "level": lv, "level_name": lv_name,
            "xp_percent": round(xp_in / max(xp_need, 1) * 100),
            "streak": g.get("streak", 0),
            "new_achievements": [a["id"] for a in ACHIEVEMENTS if a["id"] in set(g["achievements"])],
        }, ensure_ascii=False),
        headers=headers,
    )
...
async def start_api_server():
    ...
    app.router.add_options("/api/dashboard", api_dashboard_handler)
    app.router.add_post("/api/dashboard/event", api_dashboard_event_handler)
    app.router.add_options("/api/dashboard/event", api_dashboard_event_handler)
