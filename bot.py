...
# ── Своё пожелание к пакету фото ─────────────────────────────────────────────
# uid -> {"buffer_id": str, "prompt": str}  — хранит текст пожелания
user_photo_prompts: dict[int, dict] = {}
# uid -> buffer_id  — флаг: бот ждёт текстовый ввод от пользователя
photo_custom_prompt_pending: dict[int, str] = {}
...
async def handle_text(message: Message, state: FSMContext):
    ...
    # ── ПЕРЕХВАТ: ожидание «своего пожелания» к пакету фото ──────────────────
    # В aiogram 3 несколько @dp.message(F.text) не образуют цепочку —
    # поэтому перехват реализован здесь, в начале основного хендлера.
    if uid in photo_custom_prompt_pending:
        buffer_id = photo_custom_prompt_pending.pop(uid, None)
        if buffer_id:
            user_prompt = (message.text or "").strip()
            if not user_prompt:
                await message.answer("⚠️ Текст запроса не может быть пустым. Напиши ещё раз:")
                photo_custom_prompt_pending[uid] = buffer_id  # возвращаем флаг
                return
            user_photo_prompts[uid] = {"buffer_id": buffer_id, "prompt": user_prompt}
            await analyze_photos_with_custom_prompt(message, uid, buffer_id, user_prompt)
            return
    # ─────────────────────────────────────────────────────────────────────────
    ...
    if uid in last_photo and last_photo[uid].get("photo_context") and not last_photo[uid].get("fusion_pending"):
        ...
        # Если фото пришло менее 5 минут назад — любой вопрос считается follow-up
        _photo_ts = last_photo[uid].get("ts", 0)
        _photo_is_fresh = (_time_global.time() - _photo_ts) < 300
        if is_followup_cmd or is_short_followup or _photo_is_fresh:
            photo_data = last_photo[uid]
            ...
            # Явно несвязанная тема И фото уже старше 5 минут — сбрасываем контекст
            last_photo.pop(uid, None)
...
def _cleanup_photo_batch(uid: int, buffer_id: str) -> None:
    ...
    # Очищаем данные «своего пожелания» если они относятся к этому пакету
    if photo_custom_prompt_pending.get(uid) == buffer_id:
        photo_custom_prompt_pending.pop(uid, None)
    stored = user_photo_prompts.get(uid, {})
    if stored.get("buffer_id") == buffer_id:
...
async def send_photo_packet_menu(message: Message, uid: int, buffer_id: str, photo_count: int, photos: list):
    ...
    Включает кнопку «✍️ Своё пожелание» для ввода произвольного промпта.
    """
    ...
    # ── Текст сообщения по ТЗ ────────────────────────────────────────────────
    text = (
        f"📸 <b>Получено {photo_count} фото</b>{overflow_note}\n\n"
        f"Выбери действие или напиши своё пожелание"
    )
    ...
    # Режим 2: Сравнение / различия (только если 2+ фото)
    if photo_count >= 2:
        buttons.append([
            InlineKeyboardButton(
                text="⚖️ Сравнение",
    ...
    # ── НОВОЕ: Своё пожелание ────────────────────────────────────────────────
    buttons.append([
        InlineKeyboardButton(
            text="✍️ Своё пожелание",
            callback_data=f"photo_packet_custom_{buffer_id}"
        )
    ])
...
async def analyze_photos_with_custom_prompt(message: Message, uid: int, buffer_id: str, user_prompt: str):
    """
    ...
    try:
        ...
        _homework_keywords = [
            "выполни", "реши", "решить", "задание", "упражнение", "ответь",
            "задачи", "вопросы", "параграф", "учебник", "домашнее", "дз",
            "по краце", "по краче", "кратко", "самое главное", "без воды",
            "сделай", "выполнить", "помоги", "помогите",
        ]
        _is_homework = any(kw in raw_prompt.lower() for kw in _homework_keywords) or not raw_prompt

        if _is_homework:
            # Режим выполнения заданий: решаем ВСЁ по порядку конкретно
            _user_note = f"\nДополнительное пожелание пользователя: {raw_prompt}" if raw_prompt else ""
            prompt_text = (
                "На фото — учебные задания (упражнения, вопросы, задачи).\n\n"
                "ВЫПОЛНИ ВСЕ ЗАДАНИЯ:\n"
                "• Прочитай фото внимательно — найди ВСЕ пункты (1, 2, 3, а, б, в и т.д.)\n"
                "• На каждый пункт дай КОНКРЕТНЫЙ ответ с номером\n"
                "• Не пересказывай условие — сразу пиши ответ\n"
                "• Если задание с расчётом — покажи решение\n"
                "• Если вопрос на рассуждение — дай чёткий ответ 2–4 предложения\n"
                "• Формат: **1.** ответ, **2.** ответ и т.д.\n"
                f"{_user_note}"
            )
        else:
            # Произвольный запрос пользователя — используем как есть
    ...
    async with ChatActionSender.typing(bot=bot, chat_id=message.chat.id):
        ...
        try:
            ...
            last_photo[uid] = {
                ...
                "ts": _time_global.time(),   # Метка времени для 5-минутного окна follow-up
            }
            if uid not in user_memory:
                user_memory[uid] = deque(maxlen=20)
...
async def photo_packet_custom_prompt_cb(callback: CallbackQuery):
    ...
    Выставляем флаг ожидания текста и просим пользователя написать запрос.
    """
    buffer_id = callback.data.split("photo_packet_custom_")[1]
    uid = callback.from_user.id

    # Проверяем что пакет ещё существует
    if uid not in photo_batches or buffer_id not in photo_batches[uid]:
        await callback.answer("❌ Пакет не найден", show_alert=True)
        return

    # Выставляем флаг ожидания текста
    photo_custom_prompt_pending[uid] = buffer_id

    await callback.answer()
    try:
        # Обновляем сообщение — убираем кнопки, просим ввод
        packet = photo_batches.get(uid, {}).get(buffer_id, {})
